...
class Kiwoom(QAxWidget):
    ...
    def _opt10062(self, rqname, trcode):
        ...
        for i in range(data_cnt):
            if df_final.shape[0] == 3000: # max output data is 3000 rows
                self.remained_data = False
                break
            temp_dict = {}
            for output in outputs:
                if df_final.shape[0] == 3000:
                    self.remained_data = False
                    break
...
for num in codes:
    print(num)
    kiwoom.set_input_value("일자",start_date)
    kiwoom.set_input_value("종목코드", num)
    kiwoom.set_input_value("금액수량구분", "2") # 수량으로 구분
    kiwoom.set_input_value("매매구분","0") # 순매수 (2 는 순매도)
